...
def end():
  time.sleep(0.5)
  while(True):
    quit = input("Pro pokračování stiskněte [R]estart, pro opuštění [L]eave ").lower()
    if quit == "l":
      os.system("cls")
      exit()
    elif quit == "r":
...
def game():
  ...
  #flipthecoin------------------------------------------------------------------------------------------------------
      if casino_choice == "konec":
        exit()
      elif casino_choice == "f":
  ...
  #---#--------------#-------#---x--#-x-x--xxx-x--x-----xxx----x-x-xxx-x--x-x-x-x-x-xx-x-x-x-x-x-x-------------------    
      if casino_choice == "h":
